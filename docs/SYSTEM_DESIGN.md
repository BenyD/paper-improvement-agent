...
### P2b Tables — as implemented (`src/lib/parse/tables.ts`)

Aligned-column table reconstruction, ported from the heuristic in Firecrawl's open-source pdf-inspector (detect_heuristic.rs + grid.rs, studied from a clone) and adapted to our span model: a line's spans split into cells at gaps ≥ 0.7× font size (word spaces are ~0.25em; column gutters are much wider), 3+ consecutive multi-cell lines form a candidate run, cell start-x positions cluster into column centers (running-mean, 18pt tolerance), and cells map to their nearest column to form a grid — validated by requiring most rows to fill 2+ columns. Reconstructed tables are emitted as markdown-table strings, so the document model, edit ops, invariants and export are untouched; the UI recognizes the shape and renders a real table. Measured on arXiv 1706.03762: 6 tables recovered, including the complexity-comparison Table 1 with correct columns.

### P2 Structure — as implemented (`src/lib/parse/structure.ts`)

Body font size = length-weighted dominant size. Title = largest-font line cluster on page 1. Bold is detected from font PostScript names (Bold/Black/Heavy/Semibold/Medi) resolved through pdfjs's font registry — the same font-name technique Firecrawl's pdf-inspector uses. Heading classification: (a) numbered lines in a larger OR bold font, plus arabic-numbered ALL-CAPS lines at body weight (ICLR style); (b) dotted-number + Title-Case lines at any size; (c) known unnumbered headings (Abstract, References, ...); (d) short standalone lines that are clearly larger or bold at body size; (e) IEEE roman-numeral ALL-CAPS headings ("I. INTRODUCTION"). Guard: any numbered candidate whose number component exceeds 99 is rejected — "2018. URL ..." and "7.3476145. URL ..." are wrapped reference lines, not sections (this guard alone recovered a 10-page reference list that previously truncated at one page). De-noising passes: title lines excluded, heading text recurring 3+ times (figure labels) rejected unless numbered/known, figure/table captions rejected outright, and merely-styled candidates on page 1 suppressed entirely — the title block spans both columns, so author and affiliation lines otherwise leak into the section tree. Paragraphs split on vertical gaps > 1.6× the median line gap, with hyphenation repair. Failure modes: `no-title`, `no-headings`, `no-abstract` — each surfaced, never guessed. Verified on arXiv 1706.03762: full hierarchy to level 3 (3.2.1 ...), zero failures.
...
Entry-start convention detected by counting line shapes: `[n]` → bracket, `n.` → number-dot, otherwise hanging-indent geometry (entries return to the leftmost margin; wrapped continuations are indented). The margin is computed per page AND column — in a two-column reference list a single global margin would glue every right-column entry onto its predecessor (the bug that once collapsed BERT to 32 of 56 entries). Entry starts recognize surname-first ("Devlin, J."), initials-first natbib ("J. Austin, A. Odena"), and organization authors ("AI@Meta.", "Anthropic."). Groups are joined with hyphenation repair. Numbered sequences are validated — a gap ("[2] → [4]") is a surfaced `sequence-gap` failure, and implausibly short segments are surfaced with their text, never dropped.

### P5 Parse & resolve — as implemented (`src/lib/parse/entry.ts`, `src/lib/sources/`)

Local extraction first: DOI and arXiv ids by pattern (including pre-2017 `abs/` ids), year (natbib letter suffixes "2024a" parsed and stripped), authors (name-shape parsers with surname-particle handling and CSL `literal` fallback — imperfect data beats dropped data), and the title: a quoted segment is the strongest signal when present (IEEE `"Title,"` including IEEE Access's doubled single quotes `‘‘Title,''`), else a structural guess (APA: the segment after the parenthesized year; otherwise the first long non-venue segment, filtering initials-heavy author continuations).
...
Style detection counts pattern hits across the body: `[n]`/`[n,m]`/`[n-m]` brackets, `⟦^n⟧` superscript tokens (emitted by P1 for raised small-font digit runs), and author-year forms — parenthetical `(Smith et al., 2020; Jones and Lee, 2019)` with semicolon groups, and narrative `Smith et al. (2020)`. The dominant pattern is the paper's citation style. Numeric markers link by list index (ranges expanded); author-year markers link by first-author surname + year against parsed CSL fields with a whole-token raw-text fallback (substring matching once turned short org names like "AI" into wildcards). Org authors keep their full token ("AI@Meta", "DeepSeek-AI"), natbib suffix groups expand ("2023, 2024a,b" → three lookups), and letter suffixes disambiguate same-author-same-year entries via the raw entry text; multiple surviving candidates are reported AMBIGUOUS, never guessed. In a non-superscript paper, superscript tokens that link to nothing are classified as footnote marks and dropped — everything else that fails to link is surfaced: orphan markers (cite nothing in the list), and never-cited reference entries. Measured on the test paper: 102 markers, style `numeric-bracket`, zero orphans.
...
As implemented (`src/lib/sources/`): every OpenAlex, Semantic Scholar and DataCite call goes through one `cachedFetchJson` gateway — per-host throttling (S2 1.1s between calls, OpenAlex 400ms), 429 retry with backoff, and a content-addressed disk cache under `data/cache/` (sha1 of URL + body) that makes every response replayable: demos work offline, re-runs cost zero API calls, and a rate-limited API surfaces as a named per-API process note rather than a generic "search unreachable". Batch endpoints (OpenAlex OR-filter, 50 DOIs/request with `select=` field trimming; S2 `POST /paper/batch`, up to 500 ids) keep parse-time resolution to a handful of requests per paper. Free-tier quota exhaustion is a first-class state, not an error: entries stay honestly "unverified" and the Retry-verification action heals them in place once quotas reset.

### Durability: what survives a refresh

Everything durable lives on disk under `data/papers/<id>/` — the parsed document, the review, edit proposals, and an archived version history. Reopening a paper costs zero tokens; nothing is ever recomputed just to render.

The one transient thing is a review run in flight, and it is checkpointed rather than queued. The run streams over SSE and aborts on client disconnect (`req.signal` — no zombie token burn), but after every completed work unit (a section scanned for missing work, an entry claim-checked) the current result is written to `review.json` with `partial: true` and a ledger of completed unit ids. A refresh therefore loses at most one unit of work: the page reloads showing the checkpointed findings with an "interrupted" notice, and Resume replays them from disk (zero model calls), then processes only the unfinished units — failed units stay off the ledger so a resume retries them. Checkpoint writes are serialized so concurrent claim-check workers cannot interleave two half-written files. A run that completes clears `partial`, and running again after completion means a fresh full pass.

A job queue was deliberately not built: one user, one process, runs of a few minutes — the failure mode worth engineering for is the accidental refresh, which the checkpoint covers for a few lines of code. The production path is the same shape one level up: detach runs from the HTTP connection into durable execution (a queue worker, or Inngest/Temporal/Cloudflare Workflows) keyed by paper id, let SSE become a subscription to a run rather than the run itself, and the checkpoint ledger already is the resume protocol such a system needs. On serverless hosting the filesystem is ephemeral, so `data/` would move to object storage for documents and a small KV/Postgres for the ledger — the storage module is the only file that would change.
